...
{
    [BurstCompile]
    public class GaussianSplatAssetCreatorLGM : EditorWindow
    {
        ...
        [MenuItem("Tools/Gaussian Splats/Create LGM GaussianSplatAsset")]
        public static void Init()
        {
            var window = GetWindowWithRect<GaussianSplatAssetCreatorLGM>(new Rect(50, 50, 360, 340), false, "Gaussian Splat Creator", true);
        ...
        {
            NativeArray<InputSplatData> adata = default;
            ...
            // Allocate new NativeArray with the correct type and count
            NativeArray<InputSplatData> splatData = new NativeArray<InputSplatData>(splatCount, Allocator.Persistent);

            // Pointer to the raw vertex data
            float* rawDataPtr = (float*)verticesRawData.GetUnsafePtr();

            // Iterate over each vertex and construct InputSplatData
            for (int i = 0; i < splatCount; i++)
            {
                InputSplatData data = new InputSplatData();
    
                // Assuming an order and that rawDataPtr points to the correct place:
                // Fill in data from the .ply file
                data.pos = new Vector3(rawDataPtr[i * (vertexStride / sizeof(float)) + 0], rawDataPtr[i * (vertexStride / sizeof(float)) + 1], rawDataPtr[i * (vertexStride / sizeof(float)) + 2]);
                data.nor = Vector3.zero; // If normal data is not provided
                data.dc0 = new Vector3(rawDataPtr[i * (vertexStride / sizeof(float)) + 3], rawDataPtr[i * (vertexStride / sizeof(float)) + 4], rawDataPtr[i * (vertexStride / sizeof(float)) + 5]);
                data.opacity = rawDataPtr[i * (vertexStride / sizeof(float)) + 6];
                data.scale = new Vector3(rawDataPtr[i * (vertexStride / sizeof(float)) + 7], rawDataPtr[i * (vertexStride / sizeof(float)) + 8], rawDataPtr[i * (vertexStride / sizeof(float)) + 9]);
                data.rot = new Quaternion(rawDataPtr[i * (vertexStride / sizeof(float)) + 10], rawDataPtr[i * (vertexStride / sizeof(float)) + 11], rawDataPtr[i * (vertexStride / sizeof(float)) + 12], rawDataPtr[i * (vertexStride / sizeof(float)) + 13]);
    
                // Set all SH components to zero
                data.sh1 = data.sh2 = /* through */ data.shF = Vector3.zero;

                // Assign to splatData
                splatData[i] = data;
            }

            // Return the populated splatData
            return splatData;
        }
